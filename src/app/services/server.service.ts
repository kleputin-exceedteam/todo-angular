...
export class ServerService {
  ...
  pushTask(name1: string): Observable<{code: number, _id?: number}> {
    return this.http.post(`${this.tasksUrl}/add`, {name: name1},
      {headers: {token: localStorage.getItem('token')}}).pipe(
      catchError(this.handleError<any>('pushTask')
    ));
  }

  changeStatus(id1: string, status1: boolean): Observable<any>{
    return this.http.patch(`${this.tasksUrl}/change_status`, {id: id1, status: status1},
      {headers: {token: localStorage.getItem('token')}}).pipe(
      catchError(this.handleError<any>('changeStatus'))
    );
  }
  deleteTask(id: string): Observable<any>{
    return this.http.delete(`${this.tasksUrl}/delete/${id}`,
      {headers: {token: localStorage.getItem('token')}}).pipe(
  ...
  deleteCompleted(): Observable<any>{
    return this.http.delete(`${this.tasksUrl}/delete_comp`,
      {headers: {token: localStorage.getItem('token')}}).pipe(
      catchError(this.handleError('deleteCompleted'))
    );
  }
  changeAllStatus(allComple: boolean): Observable<any>{
    return this.http.patch(`${this.tasksUrl}/change_all`, {all_comp: allComple},
      {headers: {token: localStorage.getItem('token')}}).pipe(
  ...
  changeName(index, newName: string): Observable<any>{
    return this.http.patch(`${this.tasksUrl}/change_name`, {id: index, name: newName},
      {headers: {token: localStorage.getItem('token')}}).pipe(
